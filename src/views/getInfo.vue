<template>
  <div class="container mfo">
    ...
    <div v-if="imagesReady">
      <a  class="btn" :href="`http://localhost:1223/downloadImage`"  >
        Download Files
      </a>
...
export default {
  ...
  data(){
    return {
      isArray: true,
      html: htmlMFO || '',
      url: `ProgramUrl.Carmoney, ProgramUrl.Webbankir`,
      title: `title, title2`,
      name: `Carmoney, Webbankir`,
      logoImg: `Carmoney, Webbankir`,
      removeImages: true,
      convertToWebp: true,
      nameImage: `Carmoney, Webbankir`,
      description: `DESC, Webbankir`,
      loading: false,
      imagesReady: false,
      imagesName: null,
      res: [],
  ...
  methods: {
    ...
    async getInfo(){
      ...
      if(this.isArray){
        const keys = ['url', 'title', 'name', 'logoImg', 'nameImage', 'description']
    ...
    fetchData({url,title,name,logoImg,nameImage,description}){
      api.post('/getInfo/', {
        ...
        let response = createProgramMfo({
          ...
          url: url || this.url,
          offersCards: res.data.data.map(item => {
            return {
              title: item.title,
              type: 'OfferType.Basic',
              commission: item.commission,
              icon: `require('images/affiliate-program/mfo/${logoImg || this.logoImg}.webp')`,
